...
public class jshapiro314ArmProblem extends JPanel implements jshapiro314PlanningProblem {
    ...
    double obstacleX=1000, obstacleY=1000;
    double obsWidth = 0;
    double obsHeight = 0;
    ...
    public jshapiro314ArmProblem (JLabel status)
    {
	this.status = status;
    ...
    public boolean satisfiesGoal (State state, double goalX, double goalY)
    {
        ArmState a = (ArmState) state;
        double x = a.getX (numNodes-1);
        double y = a.getY (numNodes-1);
        // Use a distance of at least delta.
        if ( (Math.abs(goalX-x) > delta+epsilon) || (Math.abs(goalY-y) > delta+epsilon) ) {
            return false;
        }
        return true;
    }
