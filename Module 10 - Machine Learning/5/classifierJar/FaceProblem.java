...
public class FaceProblem extends JPanel implements ClassificationProblem {
		...
		public ArrayList<Vector<Double> >[] getTrainingData(){
				...
				ArrayList<Vector<Double> >[] retVal = features.extractSet(numClasses, leftPanel.classData);
				for (int i = 0; i < retVal.length; i++) {
						for (int j = 0; j < retVal[i].size(); j++) {
								System.out.println("Number of dimensions for sample: "+retVal[i].get(j).size());
						}
				}
				return retVal;
		}
